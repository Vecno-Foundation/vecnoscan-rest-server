...
@app.get(
    "/addresses/{vecnoAddress}/full-transactions",
...
@app.get(
    "/addresses/{vecnoAddress}/transactions-count",
...
async def get_name_for_address(
    ...
    vecnoAddress: str = Path(
        description="Vecno address as string e.g. vecno:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e",
        regex=REGEX_VECNO_ADDRESS,
    ),
):
    """
    ...
    """
    async with async_session() as s:
        r = (await s.execute(select(AddressKnown).filter(AddressKnown.address == vecnoAddress))).first()
