...
async def get_transaction(
    ...
    async with async_session() as s:
        ...
        if blockHash:
            ...
            transaction = await get_transaction_from_vecnod([blockHash], transactionId, inputs, outputs)
        ...
        if not transaction:
            ...
            if block_hashes:
                transaction = await get_transaction_from_vecnod(block_hashes, transactionId, inputs, outputs)
...
@app.post(
    "/transactions/search", response_model=List[TxModel], tags=["Vecno transactions"], response_model_exclude_unset=True
)
@sql_db_only
...
async def get_transaction_from_vecnod(block_hashes, transactionId, includeInputs, includeOutputs):
    resp = await vecnod_client.request("getBlockRequest", params={"hash": block_hashes[0], "includeTransactions": True})
