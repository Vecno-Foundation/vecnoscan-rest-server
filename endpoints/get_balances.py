...
@app.post("/addresses/balances", response_model=List[BalancesByAddressEntry], tags=["Vecno addresses"])
async def get_balances_from_vecno_addresses(body: BalanceRequest):
    """
    Get balance for a given vecno address
    """
    resp = await vecnod_client.request("getBalancesByAddressesRequest", params={"addresses": body.addresses})
