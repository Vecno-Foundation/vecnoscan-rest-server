...
@app.get("/addresses/top", response_model=TopResponse | str, tags=["Top Holders"])
async def get_top_addresses(limit: int = 100,
                            offset: int = 0):
    """
    Returns information about the top Vecno wallet holders. The 'offset' parameter will skip the
...
@app.get("/addresses/topholders", response_model=DistributionResponse | str, tags=["Top Holders"])
async def get_coin_distribution(min_amount: int = 0,
                                max_amount: int = -1):
    """
    Returns amount of Vecno addresses and total amount of $VE between the given minimum and maximum
    amount of $VE. A negative value for the 'max_amount' parameter indicates no limit.
    """
    async with async_session() as s:
        if max_amount < 0:
...
@app.get("/addresses/total", response_model=TotalResponse | str, tags=["Top Holders"])
...
@app.get("/addresses/range", response_model=List[Balances] | str, tags=["Top Holders"])
